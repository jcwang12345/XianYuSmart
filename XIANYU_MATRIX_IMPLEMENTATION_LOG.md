...
## 批次 39：Wave 17 商品改价、库存与导出审计候选（v4.0.0-rc.1-hotfix.14）

- `PRD-03 / P03-03`：商品 360 的改价/库存入口接入批量预检与持久化状态机；单 SKU 真实适配采用编辑前快照、字段白名单、写后精确回读，回读失败或不一致进入结果未知。
- `PRD-04`：价格/库存沿用既有范围快照、幂等、按店限速、子项结果、安全取消、失败项重试、通知、时间线和审计；隔离 QA Mock 明确不触达平台。
- `V6-W16-QA-001`：导出审计改为按实际账号范围逐账号落账，同一请求 ID 在账号范围查询中可见，不再写 `accountId=null`。
- 安全降级：多 SKU 逐规格编辑继续阻断；真实库存 0 引导下架；平台回读不确定时不更新本地事实、不自动重试。
- Product Design：桌面商品 360、改价/库存预检、价格精度、执行通道和上下文保持通过；修复嵌套预检 Esc 连带关闭详情。390×844 因本轮浏览器安全策略限制交由独立测试补验。
