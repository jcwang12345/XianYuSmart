...
当前版本：[2.7.0](https://github.com/jcwang12345/XianYuSmart/tree/v2.7.0) · [查看更新日志](CHANGELOG.md)
...
| AI 客服可隔离 | Spring AI、租户级动态客户端与独立向量库，支持关键词、真实会话上下文、意图路由和人工接管 | AI 能力可按租户配置；议价、参数、交付和售后问题使用受约束的专用策略 |
...
本分支的 `v2.4.1` 需使用仓库根目录 Dockerfile 本地构建；以下 GHCR 地址仍是上游最后一个公开镜像 `v2.0.7`，不包含本次商品任务中心、矩阵安全状态与售后退货证据改造。
