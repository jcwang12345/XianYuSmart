...
- 初始验收基线：`v2.3.0` / `24bd766ab7b6c3048b5e876cb2a4054b0fb08265`；最新增量回归：`v2.3.4` / `aea08f6ad8b78d0e3d3393335dd78e3fb8fc3092`
- 隔离环境：`http://127.0.0.1:12401`，MySQL 5.7.18；健康接口 `UP`，版本接口 `2.3.4`
- 总体结论：**不通过，禁止出具最终回归完成声明**。
- 缺陷状态：`XYM-PRD-001～004` 已分别在 v2.3.1～v2.3.4 关闭；`XYM-PRD-005～006` 仍未关闭，其中 006 为功能缺陷，005 为 PRD-04 安全端到端可验性阻塞。
...
| P03-03 改价/库存校验 | PASS | v2.3.4 负数、0、三位小数、超上限均 400 且无 token；0.01、12.34、9999999.99 合法；旧 rowVersion 冲突 409；`XYM-PRD-004` 已关闭 |
...
| P03-09 自动化配置 | PARTIAL | v2.3.3 首次创建单测、现有配置更新、顺序/并发幂等、冲突 409、事件/审计唯一及恢复均通过，`XYM-PRD-003` 已关闭；商品/买家/人工接管优先级仍需独立场景证据 |
...
- 状态：**v2.3.3 回归通过，关闭**。首次创建默认文案单测通过；顺序与并发重放均只产生一个事件和一条审计；同 requestId 不同配置返回 409；最终五个开关恢复全 false。
...
- 状态：**v2.3.4 回归通过，关闭**。非法边界均在解析选择范围前返回 400 且无 previewToken；合法两位小数与精确上限返回有效预检。
...
- 自动化保存 500 在 v2.3.2 本轮未再出现，但当时 `XYM-PRD-003` 仍缺重复请求幂等专项；其后续关闭证据见 7.3。

## 7.3 v2.3.3 正式增量回归

- 固定版本：commit/tag 为 `bb9c115f389424718c5a43eebc20067de7e7315b` / `v2.3.3`；运行镜像 `sha256:5a76fa79998199fcdba5de8f5b98dc8638e8bfd47df874cd3fdc2977ebae0b4e`，healthy，版本接口 2.3.3。
- 顺序幂等请求 `qa-reg-003-independent-seq-v233`：首次 `idempotentReplay=false`，第二次为 true，读回值一致；事件=1、统一审计=1。
- 同 requestId 不同配置：返回 409，文案明确要求生成新 requestId，没有覆盖赢家配置。
- 并发幂等请求 `qa-reg-003-independent-concurrent-v233`：两个同时响应均 200 且 after 状态完全一致，分别为 replay=true/false；事件=1、统一审计=1。
- 数据库只读证据：QA-GOODS-0999 配置行=1，必填自动评价文案为 29 个字符（UTF-8 87 字节），没有出现首次保存 500 的缺字段状态。
- 恢复请求 `qa-reg-003-independent-restore-v233` 成功；最终五个开关均为 false。
- 干净标签快照：ProductMatrixServiceTest 12/12、AutomationSafetyTest 9/9，总计 21/21 通过。
- `XYM-PRD-003`：首次创建、已有更新、顺序/并发幂等、读回、事件/审计唯一、冲突 409 与状态恢复均通过，状态关闭。P03-09 更广的商品/买家/人工接管优先级仍保持独立未完成项，不由本缺陷关闭结论覆盖。

## 7.4 v2.3.4 正式增量回归

- 固定版本：commit/tag 为 `aea08f6ad8b78d0e3d3393335dd78e3fb8fc3092` / `v2.3.4`；运行镜像 `sha256:0a2c26f4fde5fb927168f15c5c89bd3e1e8421a8a1699e4dde9c8f43a0f47c72`，healthy，版本接口 2.3.4。
- 仅调用 `/product-matrix/batches/preview`，未创建任务：-1、0、0.001、1.234、10000000.00 均返回 400 且无 previewToken；错误分别明确为大于 0、最多两位小数及不超过 9999999.99。
- 合法边界 0.01、12.34、9999999.99 均返回 200 和 previewToken；因隔离店铺禁用，executable=0/conflict=1，未触发平台写入。
- 字符串 12.3 和 12.340 均按等值合法金额接受；后者规范化为 12.34，不存在精度丢失。
- 干净标签快照：ProductMatrixServiceTest 15/15；vue-tsc 与 Vite 337 modules 生产构建通过。
- `XYM-PRD-004`：价格正数、精度与上限校验及“非法请求无 token”均通过，状态关闭。当前会话无浏览器截图能力，前端提示和旧预检清空仅取得代码连接与生产编译证据，不冒充独立视觉 QA。
